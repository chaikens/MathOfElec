...
Ohm's law \textbf{EQUATION:}
\begin{center}$g_ev_e=r_ei_e$\end{center}
...
{\mbox{WTS}(G\backslash p)} \text{ ``Maxwell's rule'' }
\]
...
Now known: Bollobos/Riordan/Zaslavsky theory for well-definedness
all goes through.  
Their 3 conditions on (colors,initial values) generalize to 5.
Same for the theory of activities
based on linear order on the non-port-elements.

\begin{block}{In a nutshell}
The conditions $\Longrightarrow$ activities define an unambiguous Tutte function 
...
\begin{block}{Globally ordered deletion/contraction is unnecessary.}
The Gordon/McMahon computation
tree-based activity theory also generalizes. (sdc).
\end{block}
\end{frame}
