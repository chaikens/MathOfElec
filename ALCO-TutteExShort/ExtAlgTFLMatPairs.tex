...
$\mathcal{V}$ over $K$ generated by basis (set) $U$.  The following
sets of objects are all bijectively equivalent:
\begin{enumerate}
\item All decomposible $\ext{N}\in\mathcal{A}$.
\item All so-called Grassmann representitives of subspaces of $\mathcal{V}$,
  ...
  The component
  for a sequence basis elements $B$ will be denoted by $\ext{N}[B]$,
...
Let $G=\text{diag}(\cdots, g_e , \cdots )$ be a diagonal matrix of parameters
$g_e$ with entries corresponding to $e\in U$.  The Cauchy-Binet theorem
takes the form
\[
L(N; N)= |N G N^t| = \sum_{B\subseteq U}\ext{N}[B]^2g_B
\]
and this determinant satisfies the parametrized Tutte function identity:
\[
L(N ; N) = g_eL' + L''
\]
where $L'/g_e$ is the sum for bases $B$ with $e\in B$ and $L''$ the
sum with $e\not\in B$.  


If volume form is specified for each $\mathcal{A}(U')\subseteq\mathcal{A}(U)$,
we may use the resulting Hodge star operation to define an operation
corresponding to matroid duality.  We will specify this by an
alternating sign function $\epsilon$  on sequences of
elements from $U$.  To be compatible with conventions established
for oriented matroid chirotopes\cite{OMBOOK}, we 
...
functions of $\overline{B}$.

For $A\subseteq U'$, it is straightforward to
define the operation $\ext{N}\setminus A$ corresponding to matroid
deletion.  We define the corresponding contraction operation $\ext{N}/A$
as done for duality so they are compatible:
...
and $\ext{N}_\alpha$, $\ext{L}$, $\ext{L}_E$.
...
This is too simple because we did not yet define 
``minors'' which will have smaller sets in place of $E\dunion P$. The
...
\section{Algebraic Formulation and Theorem}
...
commutes with $\Is_G$ and $\Vs_R$ because we postulate $\epsilon$ commutes with
identity extended ($\ext{e}\mapsto\ext{e}$) isomorphisms
$P \leftrightarrow P_{\alpha}\leftrightarrow P_{\beta}$.
