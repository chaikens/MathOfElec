...
\end{defi}
    ...
    \begin{proof}
      ...
      \epsilon(CAD)\epsilon{CA_\sigma D}(\ext{N}/A)[BA]
      \]
...
The global orientation $\epsilon$ commutes with
...
\begin{theo}
  ...
  $\ext{N_\alpha}^{(i)}$,  $\ext{N_\beta}^{(i)}$, $\ext{N_\beta^\perp}^{(i)}$ 
  and
  sequenced $P$, $E$, $P^{(i)}$, $E^{(i)}$, with (unsequenced sets) $P=P^{(1)}\dunion P^{(2)}$, $E=E^{(1)}\dunion E^{(2)}$,
...
\begin{equation*}
  \begin{split}
    ...
    \begin{array}{c}
      \ext{N}_\alpha^{(1)}\wedge \ext{N}_\alpha^{(2)}
      \\     
      (\ext{N}_\beta^{(1)}\wedge\ext{N}_\beta^{(2)})^\perp
    \end{array} \right)
  ...
  \epsilon(S^{(1)})\epsilon(S^{(2)})\epsilon(S^{(1)}S^{(2)})(-1)^{r{\ext{N}_\beta^\perp}^{(1)} r{\ext{N}_\beta}^{(2)}}
  \\
  =& 
  ...
  \end{split}
\end{equation*}
since $(-1)^{r{\ext{N}_\beta^\perp}^{(1)} r{\ext{N}_\beta}^{(2)}} \ext{N}_\alpha^{(2)} {\ext{N}_\beta^\perp}^{(1)}
={\ext{N}_\beta^\perp}^{(1)} \ext{N}_\alpha^{(2)}$.
\[
\epsilon(S^{(1)})\epsilon(S^{(2)})\epsilon(S^{(1)}S^{(2)}) \ext{L}/E =   \Is_G(\ext{N}_\alpha^{(1)}) \wedge \Vs_R({\ext{N}_\beta^\perp}^{(1)}) \wedge
\Is_G(\ext{N}_\alpha^{(2)}) \wedge \Vs_R({\ext{N}_\beta^\perp}^{(2)})/E
\]
\[
=
\left\{(\Is_G(\ext{N}_\alpha^{(1)}) \wedge \Vs_R({\ext{N}_\beta^\perp}^{(1)}))/E^{(1)}
\wedge
(\Is_G(\ext{N}_\alpha^{(2)}) \wedge \Vs_R({\ext{N}_\beta^\perp}^{(2)}))/E^{(2)}\right\}
\epsilon(P^{(1)}P^{(2)}E)\epsilon(P^{(1)}E^{(1)}P^{(2)}E^{(2)})
\]
proves (\ref{productequation}) except for the sign correction. Choose the arbitrary sequencings
$S^{(i)}$ to obtain for it
\[
\epsilon(S^{(1)})\epsilon(S^{(2)})\epsilon(S^{(1)}S^{(2)})\epsilon(P^{(1)}P^{(2)}E)\epsilon(P^{(1)}E^{(1)}P^{(2)}E^{(2)})
\]
\[
=\epsilon(P^{(1)}E^{(1)})\epsilon(P^{(2)}E^{(2)})\epsilon(P^{(1)}E^{(1)}P^{(2)}E^{(2)})\epsilon(P^{(1)}P^{(2)}E)\epsilon(P^{(1)}E^{(1)}P^{(2)}E^{(2)})
\]
\[
=\epsilon(P^{(1)}E^{(1)})\epsilon(P^{(2)}E^{(2)})
\epsilon(P^{(1)}    P^{(2)}    E^{(1)}E^{(2)})\epsilon(P^{(1)}P^{(2)}E)
\epsilon(P^{(1)}    P^{(2)}    E^{(1)}E^{(2)})
\]
...
Observe
\[
(-1)^{r(\ext{N}_\alpha^{(2)})r({\ext{N}_\beta^\perp}^{(1)})}=\epsilon(P^{(1)}E^{(1)}P^{(2)}E^{(2)})
\epsilon(P^{(1)}P^{(2)}E^{(1)}E^{(2)})
\]
...
\begin{equation}
  \begin{split}
    \ext{L}/E =&
    (-1)^{r(\ext{N}_\alpha^{(2)})r({\ext{N}_\beta^\perp}^{(1)})}
    \epsilon(PE)\epsilon(PE^{(1)})\epsilon(E^{(2)})\\
   &\ext{L}\left( \begin{array}{c}
    \ext{N}_\alpha^{(1)}\\
        {\ext{N}_\beta^\perp}^{(1)}\end{array} \right)/E^{(1)}
   \wedge
     \ext{L}\left( \begin{array}{c}
    \ext{N}_\alpha^{(2)}\\
        {\ext{N}_\beta^\perp}^{(2)}\end{array} \right)/E^{(2)} \\
     =& \text{bla bla bla}
  \end{split}
\end{equation}
