...
\begin{prop}
  For $i=1, 2$, let $S^{(i)}$ be the ground sets for $\ext{N}^{(i)}$, pairwise disjoint.
  \begin{equation}\label{commutewedgeperp}
    (\Nsup{1}\wedge\Nsup{2})^\perp =
    \epsilon(S^{(1)})\epsilon(S^{(2)})\epsilon(S^{(1)}S^{(2)})(-1)^{r{\Nsup{1}} r\Nsup{2}}
        \NsupPerp{1}\wedge\NsupPerp{2}
    \end{equation}
\end{prop}
\begin{proof}
  Calculate componentwise, for any ordering
  $B_i\subseteq S_i$ and $\overline{B_i}\subseteq S_i\setminus B_i$:
  \begin{equation*}
    \begin{split}
      (\Nsup{1}\wedge\Nsup{2})^\perp[B_1B_2]=
      &(\Nsup{1}\wedge\Nsup{2})[B_1B_2]
    ...
    =&\left\{\NsupPerp{1}[B_1]\epsilon(\overline{B_1}B_1)\right\}
    \left\{\NsupPerp{2}[B_2]\epsilon(\overline{B_2}B_2)\right\}\epsilon(\overline{B_1}\;\overline{B_2}B_1 B_2)\\
    ...
    \end{split}
  \end{equation*}
  Since $|\overline{B_2}|=r\NsupPerp{2}$ and $|B_1|=r\Nsup{1}$,
  \[
  \epsilon(\overline{B_1}\;\overline{B_2}B_1 B_2)=(-1)^{r\NsupPerp{2}r\Nsup{1}}\epsilon(\overline{B_1}B_1\;\overline{B_2} B_2).
  \]
