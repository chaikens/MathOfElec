...
\keywords{Tutte Functions, Exterior Algebra, Electrical Networks}
...
Tutte functions are a well-known, long-standing approach
for which new applications, variations and generalizations
continue to be active research subjects.
One of the earliest Tutte function is the basis enumerator for matroids. In the
case of graphic, and more generally, regular matroids, it is, as a
well-known consequence of the Cauchy-Binet theorem, a determinant.
...
every minor is an enumerator for (signed in non-principal minor
...
vertex indexing a deleted column. (The sign depends on the sign of the
matching each such forest determines between the deleted row vertices and
deleted column vertices; of course only the identity is such a matching
when the minor is principal.) We give a key observation: \emph{Each} such
...
when are constituted as an element of an exterior algebra, satisfy the
