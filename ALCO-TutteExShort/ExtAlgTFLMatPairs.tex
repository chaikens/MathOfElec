...
\[ \ext{L}/E = r_e\left(\ext{L}\left(\begin{array}{c} \eNal\backslash e \\
...
in our definition of dual where it is seen the result is independent of the ordering.  Our concluding
step will show that the sign correction $\epsilon(S')\epsilon(S'e)=\epsilon(PE)\epsilon(PE')$
depends only on the orderings of $E$ and $E'$ given in the data of the theorem.
  ...
  With $\ext{L}(\eNal\;\; \eNbe)=\ext{L}\left(\begin{array}{c}\eNal \\ \eNbePe \end{array} \right)$, and more sign calculations:
    \begin{defi}
      ...
      \[
      \extLHorSub{E}{\eNal}{\eNbe} = \ext{L}(\eNal\;\;\eNbe)/E
      \]
    \end{defi}
