...
sets
% no name
of objects, which are all bijectively equivalent:
\begin{enumerate}
\item All decomposible $\ext{N}\in\mathcal{A}$ equipped with a ground
  set $\subseteq U$.
...
This motivation led us to find that the class on which our Tutte function
is defined is comprised of \emph{pairs} $\ext{N}_\alpha$, $\ext{N}_\beta$
from (1)-(3) with common ground sets $S$ of the form $S=P'\dunion E$,
where $P'\subseteq P$ and $P$ is a \emph{distinguished} set of
basis elements.  The Tutte function value is in the class
...
apply to the wider class of the not just decomposible exterior algebra
...
decomposible, which means they represent linear subspaces,
...
sense even when $\ext{N}$ is a arbitrary element of the exterior algebra, not necessarilly a decomposible.
Furthermore, the properties and Tutte identity theorems hold in this wider context.   Of course,
non-decomposible elements do not represent subspaces, so our matroid representation and subspace
