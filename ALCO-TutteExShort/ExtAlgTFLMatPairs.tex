...
Our results so motivated were originally announced in 2006\cite{TutteEx}, but
their extension to
pairs is new.  Aside from their being an extension to exterior algebra
of a special case of Welsh and Kayibi's Tutte-like ``linking'' polynomial of
a pair of matroids\cite{WelshKayibiLinking},
we summarize motivations.
When taking an arbitrarilly oriented labelled multigraph for
a matroid motivated model of an electrical network, one usually
uses coordinates
for voltage drops across edges and current flows through
edges.  Kirchhoff's laws postulate that such currents and voltages
belong (KCL) to the graph's cycle space and (KVL) its orthogonal complement, respectively.
However, in circuit theory applicable to electronics, some graph models are used
...
with common edge labels:  The ``voltage graph'' and the ``current graph''.
