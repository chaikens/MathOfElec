...
\begin{defi}
  ...
  $\Is_G(\ext{p_j})=\ext{p_{\alpha j}}$, and   $\Is_R(\ext{p_j})=\ext{p_{\beta j}}$.
\end{defi}
...
\begin{defi}
  \begin{gather}
    ...
    \LVertSub{E}{\eNal}{\eNbePe} = \Is_G(\ext{N_\alpha})\wedge\Vs_R(\ext{N_{\beta}^\perp})/E\\
    \LHor{\eNal}{\eNbe}=\extLVert{\eNal}{\eNbePe}\\
    \LHorSub{E}{\eNal}{\eNbe}=\extLVertSub{E}{\eNal}{\eNbePe}
  \end{gather}
\end{defi}
...
%  {\Large REMARK: $\ext{L}_E(\eNal\;\;\eNbe)$ is the exterior product of the rows of
...
=\Is_G(\eNal)\wedge\Vs_R(\eNbePe)
  ...
  In each factor, let us group the terms according to those that don't and those that do
...
\]
Each factor has the form $\{X + Y\ext{e}\}$ where $X$ (and $Y$) are not (exterior) multiples of $\ext{e}$, so
\[
\ext{L}=
...
\end{split}
\]
With sets $E$ (and $E'=E\setminus e$) given by ordered sequences
...
$S'$ is an arbitrary ordering of $P_\alpha \dunion P_\beta \dunion (E\backslash e)$ introduced
in our definition of $\ext{N}^\perp$ where it is seen that $\ext{N}^\perp$ is independent of this
ordering.  Our concluding
step will show that the sign corrections in (\ref{delecontrequation}) depend only
on the orderings of $P$, $E$ and $E'$ from theorem's data, that is
we must show (\ref{delecontrDEPSprime})
$\ext{L}_E=\epsilon(S')\epsilon(S'e)(\ext{L'}\wedge \ext{e})/E$
implies (\ref{delecontrequation}).

The only relevant exterior algebra monomials in $\ext{L'}$ are multiples of $\wedge E'=\ext{E'}$,
the others will be sent to zero by the $/E$ operation.
Permute the vectors in each such monomial expression so it
has the form $\ext{X}\ext{E'}$ and set the sign to keep the expression value unchanged.
So (\ref{delecontrDEPSprime}) is expressed by $\epsilon(S')\epsilon(S'e)(\ext{L''}\ext{E'}\ext{e})/E$, and

\[
\ext{L}_E=\epsilon(S')\epsilon(S'e)(\ext{L''}\ext{E'}\ext{e})/E
=\epsilon(S')\epsilon(S'e)\epsilon(E'e)\epsilon(E)(\ext{L''}\ext{E})/E.
\]
Since the ordering $S'$ is arbitrary, take $S'=PE'$ so
\[
\ext{L}_E = \epsilon(PE')\epsilon(PE'e)\epsilon(E'e)\epsilon(E)(\ext{L''}\ext{E})/E.
\]
We can now simultaneously permute the two appearances of subsequence $E'e$ into $E$, which
keeps the above value unchanged.  The proof concludes with
\[
