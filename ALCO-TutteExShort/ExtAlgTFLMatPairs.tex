...
objects to be accompanied by a ground set.  In order
to investigate Tutte functions on $\mathcal{N}$, we also
must define operations corresponding to defining minors and duals.
...
rows and columns respectively, we postulate disjoint copies $P_\alpha$ and
$P_\beta$, with bijections
...
$|N G N^t|$
where $G=\text{diag}(\cdots, g_e , \cdots )$ be a diagonal matrix of parameters
...
functions of $\overline{B}$.  The volume forms for distinct sets
...
as done for duality so they are compatible.  (It can be
defined in terms of interior product\cite{MarcusFDMuAlPt2}, except for our
...
we illustrate in terms of matrices.
