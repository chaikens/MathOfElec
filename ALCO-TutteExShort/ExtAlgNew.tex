...
bases. Such a base $S$ can be identified with a matroid's ground set.
Later we explore consequences of further distinguishing some basis elements
$p\in P$ (we call ``ports'')
so deletion and contraction are restricted away from such
$p$\cite{MR0419272,SetPointedLV,sdcPorted,TutteEx,RelTuttePolyDiaoHetyei}.
It does no
harm to take the concrete point of view and consider elements of
$S$ to be coordinates; more accurately, \emph{names} or
\emph{labels} for coordinates.  This is how a matrix $N$ represents a matroid
whose ground set $S$ is comprised of the labels of the matrix columns.  This matroid is
also represented the subspace $\mathcal{N}$ of $KS$ (over $K$)
generated by the (linear combinations over $S$ encoded by the) rows.  Therefore,
$N$ will be assummed to have full row rank.
...
of the rows of $N$.  The next simple
