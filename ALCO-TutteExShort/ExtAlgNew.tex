...
grounds for multiple analogies and for studies of convolutions.
It might therefore be of interest to see some Tutte functions valued
in exterior algebras.  Unlike the classical and recently employed frameworks
we are aware of, 
exterior algebras have \emph{anticommutative} multiplication.

Our construction depends on equipping vector spaces with particular
bases and further distinguishing some of their elements.  It does no
harm to take the concrete point of view and consider these basis
vectors to be coordinates; more accurately, \emph{names} or
\emph{labels} for coordinates.  This is how a matrix $N$ represents a matroid
whose ground set $E$ is comprised of the labels of the matrix columns.  This matroid is
represented just as well within the space generated by the column labels,
as the subspace $\mathcal{N}$ generated by the (linear combinations encoded by the) rows.

Our first necessary step is take to consider the pure tensor $\mathbf{N}$,
determined up to non-zero multiples by $\mathcal{N}$, defined
by the exterior product 
of the rows of $N$ after we take $N$ to have full row rank.  The result is the simple
decomposition that begins but does not completely fulfill a Tutte function analogy:
\[
\text{for all } e \in E, \mathbf{N} = \mathbf{e}\wedge\mathbf{N}/\mathbf{e} +
