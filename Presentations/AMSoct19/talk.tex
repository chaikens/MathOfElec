...
\end{document}
  ...
  The $k$-cochains $C^k = \text{Hom}(C_k,\COEF)$ is the space of linear maps
  from $C_k$ to a coefficient ring $\Reals$.\\
  The $k$-complex $X=\dunion_{i=0}^{k}X_k$ ($X_k$ is the set of $k$-simplices) determines,
  ...
  The dual $\partial^{j}:C^{j}\rightarrow C^{j+1}$ is defined by
  %  $(\partial^j(u^*))(v)=u^*(\partial^
