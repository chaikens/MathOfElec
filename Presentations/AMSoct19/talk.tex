...
\]
The $A$ term is $\neq 0$ iff $Q_\alpha A$ is a column basis for $\Nal$\\
and $Q_\beta \overline{A}$ is a column basis for $\NbePe$.
...
%    \\
 ...
 %     (a_2b_3-a_3b_2)\mathbf{e}_2\mathbf{e}_3)}
  ...
  \end{array}
  \]
  We will omit the $\wedge$ and we can always write:
  \[
  ...
  is the exterior product of (row coordinate vectors) \textbf{in the same order}
  \[
  \ext{A} = \ext{a}_1\ext{a_2}\ldots\ext{a_r}
  \]
\end{frame}

\begin{frame}{Catalog of Oriented Matroid operations on matrix $N$ \& $\ext{N}=\wedge(\text{rows}(N))$}
    \[
    \begin{array}{ccc}
\text{type of representation} &  \text{chirotope}            & \wedge(S) \text{product} \\
\text{value}                & \chi(B)=\text{sign}(N[B]) & \ext{N},\text{components\ }\ext{N}[B] \\
\text{deletion\ } \bullet\backslash e  & \chi'=\chi|S\backslash e  & \ext{N\backslash e}[B] = \ext{N}[B], e\not\in B \\
\text{contraction\ }\bullet / e              & \chi'(B) = \chi(Be) & \ext{N/e}[B] = \ext{N}[Be] \\
\text{type of value}                   & \text{sign}\in\{0,+,-\} &  \text{\bf field value; number}\\

\text{duality\ }\bullet^{\perp} & \chi^{\perp}(B) = \chi(\overline{B})\epsilon(\overline{B}B) &
