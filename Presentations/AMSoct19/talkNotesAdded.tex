...
\begin{block}{Pl\"{u}cker coordinates}
  These determinants can be considered a \emph{vector space} version of the
  (projective space) Pl\"{u}cker coordinates for the row space of $L$ projected
  into $K^{P_\alpha\dunion P_\beta}$.  We need \emph{vectors} to add so Tutte's $\mathbf{+}$ identity makes sense.
\end{block}
\end{frame}
