...
\item An extensor valued Tutte function and\\
 a (linear) resistance network's
behavior projected on fixed distinguished coordinates.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Example}
Matrix $M$ of the electrical network equations $Mx=0$.   Kirchhoff's laws
apply to all cycles and cocyles with $r_ix_{e_i}$ as voltage and 
and $g_ix_{e_i}$ as current of resistor (not port) edges.  SEPARATE voltage and
current variables for each port edge.
\[
...
the $2|P|$ columns $\{ip_k, vp_k\}$, the equation holds for all $e_i\not\in P$:
