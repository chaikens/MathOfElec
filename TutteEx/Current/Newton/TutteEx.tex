...
%Used for Extensor/Matroid Pair theory..
...
%\renewcommand{\@oddfoot}{\footsc paper in preparation for submission
\renewcommand{\@oddfoot}{\footsc Working draft (Newton)
   (2008), $Revision: 1.4 $ |\#R00\hfil\footrm\thepage}}
\makeatother
\pagestyle{plain}
...
\newpage
%\noindent\verb|$Id: TutteEx.tex,v 1.4 2009/05/08 21:13:41 seth Exp seth $|
...
\rho_{11}=-\frac{\ext{M}_E(\ext{N})[i_1v_2]}{\ext{M}_E(\ext{N})[v_1v_2]}
Extensor $\ext{M}_E(\ext{N})$ of 
Definition \ref{DefM} is a representation of the linear 
subspace of constraints whose solution set 
$M^{\perp}\subseteq\Reals^{P_{\Is}\dunion P_{\Vs}}$ comprises all feasible
combinations of voltage and current values at the network ports.  
When $\ext{M}_E(\ext{N})$ is written as the exterior product 
of step 1 extensors, 
the factors are easily written as the rows of a matrix whose columns are
indexed by $P_{\Is}\dunion P_{\Vs}$.  Then 
$M^{\perp}=\{m | \ext{M}_E(\ext{N})m=0\}$.  
Continuing our example:
\[
\begin{array}{cc}
\omN & \text{(matrix)} M_\emptyset(\omN) \\ \hline
\omN_0(p_1)\oplus \omN_1(p_2) & 
   \begin{array}{cccc} 
       i_1 & i_2 & v_1 & v_2  \\  \hline
        0  &  1  &  0  & 0 \\
        0  &  0  &  1  & 0 
   \end{array}  \\ \hline
\omN_1^+ &
...
\textbf{Derivation of Theorem \ref{Maxwell1} and its Corollary:}
Let $\ext{M}_E(\ext{N})$ be the extensor from 
Definition \ref{DefM} for the two port 
network of Theorem \ref{Maxwell1}.  Applying Proposition \ref{PropositionMain}
with all $r_e=1$, our hypothesis about $T$ implies 
$\epsilon(p_1p_2)\ext{M}_E(\ext{N})[v_1v_2]=\sum g_T>0$.  Hence the equations
$\ext{M}_E(\ext{N})(1\; 0 \; \rho_{11}\; \rho_{21})^t=0$ have a unique solution.
...
The Kirchhoff enumeration methods include 
...
above determinant.
%It is helpful to know that for graphs with $P=\emptyset$
%the main function constructed in this paper
%reduces to the reduced Laplacian determinant.  
When $P$ is 
...
(see \cite[\textsection 12.4]{LinNonLinCircuitsBook}) equivalent to 
our extensor function are routine 
linear algebra exercises used in electrical network analysis. 

The complete background is 
given in \textsection \ref{Preliminaries}.
The function of Theorem \ref{TheoremMain}
is defined on certain extensors and that has
...
For brevity we will call all extensor-valued functions 
\textbf{extensor functions}.
